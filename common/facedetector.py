...
class FaceDetector:
    """ Face Detector class
    """
    def __init__(self, prototype: str=None, model: str=None,
                 confidenceThreshold: float=0.5):
        self.prototype = prototype
        self.model = model
        self.confidenceThreshold = confidenceThreshold
        if self.prototype is None:
            raise FaceDetectorException("must specify prototype '.prototxt.txt' "
                                        "file path")
        if self.model is None:
            raise FaceDetectorException("must specify model '.caffemodel' file path")
        self.classifier = readNetFromCaffe(self.prototype, self.model)
